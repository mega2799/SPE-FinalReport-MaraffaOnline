...
\subsection{Struttra delle reposistory}

Data la natura del sistema, sono state necessarie numerosi repository git per evitare ulteriori confusioni, separare correttamente i componenti e poterli rilasciare indipendentemente.
Il sistema però è comunque utilizzabile e rintracciabile attraverso la repository gitlab di ateneo PikaLab perche è stata implentatata una repository con submodules. 
Inoltre in questa repository si può trovare il docker-compose per il deploy del sistema completo e sempre aggiornato assieme ad alcuni file statici per il suo funzionamento o configurazione.
