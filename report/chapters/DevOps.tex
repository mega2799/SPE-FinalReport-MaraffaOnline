...
Nginx servirà per poter eseguire l'applizazione all'interno del container e per poterla raggiungere dall'esterno. Per fare ciò è stato necessario configurare Nginx tramite un 
file di configurazione con il quale è stato anche possibile gestire un operazione di reverse proxy per poter indirizzare le chiamate al backend.

\begin{lstlisting}[language=Python, caption={Nginx del front-end}, label=list:nginx_frontend]
    server {
    listen 80;
...
L'indirizzo del backend è stato configurato tramite variabili d'ambiente, in modo da poterlo cambiare facilmente in base all'ambiente in cui si trova il container.
Che vengono poi sostituite a seconda dell'ambiente di deploy in cui l'applicativo in angular viene eseguito. 
