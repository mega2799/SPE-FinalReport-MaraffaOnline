...
\section{Checkstyle, javadoc, github page [NON so se va bene qua ma lo scrivo] }
...
Per lo sviluppo di questa, e di future relazioni, è stata creata una CI/CD. Questa è poi stata inclusa a sua volta in un template github appositamete nato per lo sviluppo di nuove relazioni. Contiene una serie di documenti in Latex, che rappresentano una scheletro per le relazioni e la CI per poter 
pubblicare la relazione in continuos integration in formato HTML sulla piattaforma GitHub Pages e fornire una release contente il PDF della relazione. L'utilizzo di github rende lo sviluppo della relazione più semplice, 
non si è obbligati a scegliere strumenti come overlaeaf come editor di testo, la relzione è versionata e si può lavorare in parallelo con altri membri del team sfruttando tutte le comodità offerte da git. 

A questo \href{https://github.com/mega2799/Report-ghPages-latex}{\underline{link}} il template spiega alcuni dei passaggi per poterlo utilizzare. In alternativa 
è stato inserito anche un file bash che permette di poter, tramite un container docker, creare localmente il pdf della relazione utlizzando i software pandoc e latexmk.

Sono stati fondamentali i lavori \href{https://github.com/MemerGamer/LaTex-Publishing}{\underline{Memer Gamer repo}} per la base del workflow delle pubblicazioni su gituhb pages e \href{https://www.overleaf.com/latex/templates/university-of-reading-computer-science-report-template-and-guide/xhttddjhkwrf}{\underline{Varun Ojha per il template}}.
