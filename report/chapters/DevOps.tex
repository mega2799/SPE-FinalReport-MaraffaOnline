...
\subsection{Testing}
All'interno della CI la fase di test è stata implementata con Jest per i servizi che usano nodeJS genrando di fatto un report abbastanza comprensibile direttamente nella finestra della CI. Il componente middleware, scritto in java invece, 
generava risultati di test non così chiari. Per modificare questo comportamento è stata introdotta una dipendenza nel build gralde che permette di avere un report più dettagliato e comprensibile. 
La libreria \href{\underline{Test Logger}}{https://plugins.gradle.org/plugin/com.adarshr.test-logger} tramite una piccola configurazione nel file build.gradle come qui riportata ha soddisfatto le nostre esigenze.
